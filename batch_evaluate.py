...
def main():
	...
	for ckpt in ckpts:
		...
		filewriter.writerow(['Case', 'TP', 'FP', 'FN', 'Sensitivity', 'IoU'])

		for case in os.listdir(dataDir):
			if not os.path.exists(os.path.join(dataDir,case,"label_vnet.nii.gz")):
				continue
			print("case:",case)
			_TP, _FP, _FN = Accuracy(os.path.join(dataDir,case,"label_crop.nii.gz"),os.path.join(dataDir,case,"label_vnet.nii.gz"))
			TP += _TP
			FP += _FP
			FN += _FN
			...
			filewriter.writerow([case, _TP, _FP, _FN, _sensitivity, _iou])
		csvfile.close()
